...
This repository ships the `@nanhara/hara-video` npm package and Hara plugin. The CLI entry point is `bin/hara-video.mjs`; production helpers are in `scripts/`; the staged production workflow, recipes, templates, and references are in `skills/video/`; the account-isolated distribution workflow is in `skills/video-publish/`; tests are in `test/`. Keep `package.json` and `plugin.json` versions aligned.
...
Treat image/TTS backend templates, publication adapters, profiles, and project media as untrusted. Public skills may define schemas and secret-free examples, but must never contain real brand routing, account identifiers, credential paths, cookies, API keys, voice credentials, private media, biometric voice data, tokens, passwords, or authorization headers. Keep live publication profiles outside this repository and redact commands before logging or reporting them.
